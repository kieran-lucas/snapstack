...
The overlay now paints a dim layer outside the active rectangle, keeps the chosen pixels clear, and uses a blue selection border. A visual-check-only environment switch allows the benchmark to photograph the overlay without capture exclusion; the sample confirmed the dim/clear split. Normal capture still excludes overlay windows as a secondary safeguard. A post-change 20-selection native-core run completed with exact 800 × 500 dimensions, release → pixels **6.08 / 22.73 / 31.58 ms** median / p95 / p99. The tail regressed relative to the earlier 14.71 ms p95, so it should be retested in-app before attributing that difference to the visual change.
